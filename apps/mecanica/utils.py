...
from mecanica.models import Cuenta

def generic_upload(path, f):
    path = 'uploads/{0}/'.format(path).replace('//','/')
    if not os.path.exists(os.path.join(settings.MEDIA_ROOT, path)):
        os.mkdir(os.path.join(settings.MEDIA_ROOT, path))
    with default_storage.open(path+f.name, 'wb+') as destination:
...
def get_cuenta_session(request):
    try:
        return Cuenta.objects.get(pk=request.session.get('_cuenta_id', ''))
    except Cuenta.DoesNotExist:
        return False

def create_cuenta_session(request, user):
    cuenta_id = user.user_perfil.cuenta.id
    request.session['_cuenta_id'] = cuenta_id
    return cuenta_id
